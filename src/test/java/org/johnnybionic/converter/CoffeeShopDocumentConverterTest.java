...
public class CoffeeShopDocumentConverterTest {
	...
	public void thatExtraAttributesAreReturned() {
		...
		Document source = getSource().append(ATTRIBUTE_1, ATTRIBUTE_1).append(ATTRIBUTE_2, ATTRIBUTE_2);
		CoffeeShop result = converter.convert(source);
		assertNotNull(result);
		assertNotNull(result.getAttributes());
	...
	public void thatExceptionOccursForMissingCoordinates() {
		Document source = new Document(CoffeeShopConverter.ID_KEY, TEST_ID);
		converter.convert(source);
	}

	private Document getSource() {
		ObjectId id = new ObjectId(TEST_ID);
		Document source = new Document(CoffeeShopConverter.ID_KEY, id);
		source.put(CoffeeShopConverter.NAME_KEY, TEST_NAME);
		
		List<Double> coordinates = new ArrayList<>();
		coordinates.add(LONGITUDE);
		coordinates.add(LATITUDE);
		Document location = new Document(CoffeeShopConverter.COORDINATES_KEY, coordinates);
