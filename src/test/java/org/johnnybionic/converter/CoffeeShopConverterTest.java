...
/**
 * The converter is used to create {@link CoffeeShop} instances from documents returned
 * from the coffeeshops collection. The main reason it exists is to convert the location into
 * a {@link Coordinates} instance, as the default converter(s) can't do that.
 * 
 * @author johnny
 *
 */
public class CoffeeShopConverterTest {
	...
	public void thatCoffeeShopIsCreated() {
		BasicDBObject source = getSource();
		
		CoffeeShop result = converter.convert(source);
		assertNotNull(result);
		
		assertEquals(TEST_NAME, result.getName());
		assertEquals(TEST_ID, result.get_id());
		...
		// there should be three attributes - the ones that are explicitly set as members of the CoffeShop
	...
	public void thatExtraAttributesAreReturned() {
		...
		BasicDBObject source = getSource().append(ATTRIBUTE_1, ATTRIBUTE_1).append(ATTRIBUTE_2, ATTRIBUTE_2);
		CoffeeShop result = converter.convert(source);
		assertNotNull(result);
		assertNotNull(result.getAttributes());
	...
	public void thatExceptionOccursForMissingCoordinates() {
		BasicDBObject source = new BasicDBObject(CoffeeShopConverter.ID_KEY, TEST_ID);
	...
	private BasicDBObject getSource() {
		...
		BasicDBObject location = new BasicDBObject(CoffeeShopConverter.COORDINATES_KEY, coordinates);
